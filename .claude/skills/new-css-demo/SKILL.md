...
2. **Copy the template.** Copy `references/template.html` to `<slug>.html` at
   ...
   full caniuse support section as clonable patterns. It also already carries
   the no-flash theme snippet and links the shared `theme.js` (dark-mode
   toggle) and `toc.js` (sticky TOC) — don't remove those `<script>` tags.
...
4. **Build 3-6 numbered sections.** Each section is `<p class="num">NN /
   ...
   inputs bound via the template's `rangeBind`/`selBind` helper, see its demo
   ...
   existing demos (`rangeBind`/`selBind`).
...
The repo-wide rules already live in `CLAUDE.md` and aren't repeated here:
link the shared `styles.css` and inline only feature-specific CSS, one file
per demo with no build step or framework, the index is newest-first with no
`idx` badges, and commit messages stay plain with **no** `Co-Authored-By` or
other AI-attribution trailer. The one convention specific to authoring a demo:

- **Always use the palette vars** (`var(--paper)`, `var(--accent)`, etc.) for
  any colour a demo needs, never a bare hex. `styles.css` flips those vars via
  `:root[data-theme="dark"]`, so a demo built on them — and its `.specimen`
  card — goes dark automatically; a hardcoded colour won't adapt and can
  silently go low-contrast (dark text invisible once the card goes dark).
  Don't hand-roll a per-page dark variant. If a demo genuinely needs a fixed
  tint no var can express, add an explicit `:root[data-theme="dark"]
  .your-selector { … }` counterpart next to it, the way `has-selector.html`'s
  checked-card highlight does.
